...
def test_verify_slip_success(client):
    with (
        patch("app.services.firestore.get_booking", return_value=_BOOKING_PENDING),
        patch("app.services.slipok.verify_slip_file", new=AsyncMock(return_value=_SLIPOK_OK)),
        patch("app.services.firestore.is_trans_ref_used", return_value=False),
        patch("app.services.storage.upload_slip", new=AsyncMock(return_value="slips/booking-001/1.jpg")),
        patch("app.services.firestore.update_booking"),
...
def test_verify_slip_duplicate_trans_ref(client):
    with (
        patch("app.services.firestore.get_booking", return_value=_BOOKING_PENDING),
        patch("app.services.slipok.verify_slip_file", new=AsyncMock(return_value=_SLIPOK_OK)),
        patch("app.services.firestore.is_trans_ref_used", return_value=True),
    ):
        resp = _post_slip(client)
    assert resp.status_code == 422
...
def test_verify_slip_wrong_status(client):
    ...
    with patch("app.services.firestore.get_booking", return_value=booking_confirmed):
        resp = _post_slip(client)
