...
def test_create_booking_success(client):
    with (
        patch("app.services.database.create_booking", return_value="booking-001"),
...
def test_create_booking_full_slot(client):
    with patch(
        "app.services.database.create_booking",
...
def test_get_booking_success(client):
    with patch("app.services.database.get_booking", return_value=_BOOKING_RAW):
...
def test_cancel_booking(client):
    with (
        patch("app.services.database.cancel_booking"),
        patch("app.services.database.get_booking", return_value=_BOOKING_RAW),
